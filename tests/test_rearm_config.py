...
class RearmConfigurationTest(unittest.TestCase):
    def test_timer_defaults_to_twenty_seconds(self):
        config = bridge.CONFIG_SCHEMA({})
        self.assertEqual(config["rearm_after"].total_milliseconds, 20000)

    def test_custom_delay_and_explicit_disable(self):
        for value, expected in [("20s", 20000), ("1250ms", 1250), ("0s", 0)]:
            with self.subTest(value=value):
                config = bridge.CONFIG_SCHEMA({"rearm_after": value})
                self.assertEqual(config["rearm_after"].total_milliseconds, expected)
        ...
        for value in ["-1s", "100us", "30d", "not-a-duration"]:
            with self.subTest(value=value):
                with self.assertRaises(cv.Invalid):
                    bridge.CONFIG_SCHEMA({"rearm_after": value})
