...
// State of the emulated lock, used to rearm Vision's passive face/palm unlock.
// This is not feedback from a physical lock. Kept apart
...
class KeypadState {
 public:
  ...
  void set_rearm_after(uint32_t ms) { this->rearm_after_ms_ = ms; }
  uint32_t rearm_after() const { return this->rearm_after_ms_; }
  ...
  void unlock(uint32_t now) {
    ...
    this->rearm_pending_ = this->rearm_after_ms_ != 0;
  }

  // Unsigned elapsed time remains correct across millis() rollover.
  // Returns true once per expiry, for logging by the bridge.
  bool update(uint32_t now) {
    if (!this->rearm_pending_ || this->rearm_after_ms_ == 0 ||
        static_cast<uint32_t>(now - this->unlocked_at_) < this->rearm_after_ms_) {
      return false;
    }
    this->rearm();
    return true;
 ...
 protected:
  ...
  uint32_t rearm_after_ms_{20000};
